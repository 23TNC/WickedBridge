...
# sex.appropriate 2237 calls and birthcontrol.condom 46. Keep callbacks on a
# HOT gate cheap: no relationship walks, no tuning lookups.
#
# REMOVED in 0.7.0: sex.allow_sim, over is_sim_allowed_for_sex. It read like
# "may this Sim have sex" and was documented that way, but it is a broad
# eligibility predicate -- pubic hair growth, lewd dreams, desire, penis-size
# dialogs and the attractiveness selector all consult it. On the paths that
# actually start an act (_StartSexInteraction._is_sex_allowed, JoinInSex,
# InviteInSex) it is only ever asked about the Sim PERFORMING the interaction,
# never the partner, so vetoing it denied a dozen unrelated features while a
# partner could still be pulled into sex. Verified in the game: 21 vetoes
# recorded, sex proceeded. A gate nothing can enforce through is worse than no
